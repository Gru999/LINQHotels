...
var query4 = from room in rooms
             where (room.Type.Equals("D") && room.Price < 400)
             select room;
...
var query5 = from room in rooms
             where ((room.Type.Equals("D") || room.Type.Equals("F")) && room.Price < 400)
             orderby room.Price
             select room;
...
var query6 = from hotel in hotels
             where hotel.Name.Substring(0, 1).Equals("P")
             select hotel;
